class GameOver extends Phaser.Scene {
    ...
    create() {
        ...
        if(currentScene =='playScene1'){
            ...
            this.add.text(centerX, 350, '[ Kindergarten Score: '+performance1+' ]', scoreConfig).setOrigin(0.5);
            if(performance1>=0 && performance1<20){
                this.add.text(centerX, 150, 'Gotcha! You need some practice, Kid!', feedbackConfig).setOrigin(0.5);
            }else if(performance1>=20 && performance1<60){
                this.add.text(centerX, 150, 'Almost, almost!', feedbackConfig).setOrigin(0.5);
            }else if(performance1>=60 && performance1<=105){
                this.add.text(centerX, 150, 'Well-Done! Sharp-Shooter!', feedbackConfig).setOrigin(0.5);
            }
        }
        if(currentScene =='playScene2'){
            ...
            this.add.text(centerX, 370, '[ High School Score: '+performance2+' ]', scoreConfig).setOrigin(0.5);
            if(performance2>=0 && performance2<20){
                this.add.text(centerX, 150, 'Shuuuu! Silence! Boy!', feedbackConfig).setOrigin(0.5);
            }else if(performance2>=20 && performance2<60){
                this.add.text(centerX, 150, 'You almost had it all~', feedbackConfig).setOrigin(0.5);
            }else if(performance2>=60 && performance2<=105){
                this.add.text(centerX, 150, 'Good job! You ate them all!', feedbackConfig).setOrigin(0.5);
            }
        }
        if(currentScene =='playScene3'){
            this.backGround = this.add.sprite(0,0,'transition(2)').setOrigin(0,0).setAlpha(0.8);
            this.backGround.anims.play('office');
            this.add.text(centerX, 400, '[ Adult Score: '+performance3+' ]', scoreConfig).setOrigin(0.5);
            if(performance3>=0 && performance3<20){
                this.add.text(centerX, 150, 'OHHH, you might got fired for that! Man!', feedbackConfig).setOrigin(0.5);
            }else if(performance3>=20 && performance3<60){
                this.add.text(centerX, 150, 'You almost killed that dragon!', feedbackConfig).setOrigin(0.5);
            }else if(performance3>=60 && performance3<=105){
                this.add.text(centerX, 150, 'Wow, immpresive! Congrats!', feedbackConfig).setOrigin(0.5);
            }
        ...
        this.add.text(centerX, 250, reason, feedbackConfig).setOrigin(0.5);
